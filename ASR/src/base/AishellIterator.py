...
class AishellIterator(DataIterator):

  def __init__(self):
    super(AishellIterator, self).__init__()
  ...
  def configure(self, data_root, transcripts):
    ...
    # traverse the data root, find wav files and get its label
    for root, dirlist, filelist in os.walk(data_root):
      for filename in filelist:
        if filename.endswith('.wav'):
          base_name = filename.split('.')[0]
          labels = file_labels_dict.get(base_name, None)
          if labels is not None:
            relative_path = os.path.join(root, filename).replace(data_root+'/', '')
            self.data_list.append([relative_path, labels])
            if relative_path.startswith('train'):
              self.train_list.append([relative_path, labels])
            elif relative_path.startswith('dev'):
              self.val_list.append([relative_path, labels])
            elif relative_path.startswith('test'):
              self.test_list.append([relative_path, labels])
