...
digit_chara_dict = {'1' : u'一', '2' : u'二', '3' : u'三', '4' : u'四', '5' : u'五',
                    '6' : u'六', '7' : u'七', '8' : u'八', '9' : u'九',
...
class JDDIterator(DataIterator):
  ...
  def __init__(self):
    ...
    """
    super(JDDIterator, self).__init__()
  ...
  def configure(self, root, path):
    ...
    with open(path, 'r', encoding='utf-8') as f:
      for aline in f.readlines():
        filename, labels = aline.strip().split('.wav\t')
        labels = [label for label in labels if label not in [u' ', u',', u'.', u':', u'。', u'，']]
        # let '1' -> u'一', 'a' -> 'A'
        labels = [digit_chara_dict.get(label, label) for label in labels]
        words.extend(labels)
