...
# Filename: bottleNeck.py
# Description: construct bottleneck network, cnn is composed of 1d residual network
# *************************************************
...
def bottleneck_1d(inputs, kernel_size, reduce_dim, out_channels, dilation_rate, is_training, keep_prob):
  ...
  param reduce_dim : integer, reduced dimensio
  n
  ...
  with tf.name_scope("bottleneck/%d" % bottleneck_cnt):
    ...
    atrous_conv_bn = tf.layers.batch_normalization(atrous_conv, training=is_training)
    atrous_conv_relu = tf.nn.relu(atrous_conv_bn)

    # atrous gate
    atrous_gate_conv = tf.layers.conv1d(reduce_relu, filters=out_channels, kernel_size=kernel_size, padding='same', dilation_rate=dilation_rate, use_bias=False, kernel_initializer=tf.contrib.layers.xavier_initializer())
    atrous_gate_bn = tf.layers.batch_normalization(atrous_gate_conv, training=is_training)
    atrous_gate_sigmoid = tf.nn.sigmoid(atrous_gate_bn)

    atrous_gated_outputs = atrous_conv_relu * atrous_gate_sigmoid
    
    # restore the dimension
    restore_conv = tf.layers.conv1d(atrous_gated_outputs, filters=out_channels, kernel_size=1, use_bias=False, kernel_initializer=tf.contrib.layers.xavier_initializer())
...
def bottleNeck(inputs, is_training, NUM_LABELS):
  """
  ...
  with tf.name_scope("bigblock/1"):
    bigblock_1_outputs = bigblock_1d(inputs_relu, num_block=5,
                                     kernel_size=kernel_size, reduce_dim=64,
                                     out_channels=128, is_training=is_training,
                                     keep_prob=0.8)

  # bigblock 2
  with tf.name_scope("bigblock/2"):
    bigblock_2_outputs = bigblock_1d(bigblock_1_outputs, num_block=5,
                                     kernel_size=kernel_size, reduce_dim=64,
                                     out_channels=128, is_training=is_training,
                                     keep_prob=0.8)
  ...
  with tf.name_scope("bigblock/3"):
    bigblock_3_outputs = bigblock_1d(bigblock_2_outputs, num_block=5,
                                     kernel_size=kernel_size, reduce_dim=128,
                                     out_channels=256, is_training=is_training,
                                     keep_prob=0.8)

  # bigblock 4
  with tf.name_scope("bigblock/4"):
    bigblock_4_outputs = bigblock_1d(bigblock_3_outputs, num_block=5,
                                     kernel_size=kernel_size, reduce_dim=128,
                                     out_channels=256, is_training=is_training,
                                     keep_prob=0.8)
  ...
  #   bigblock_5_outputs = bigblock_1d(bigblock_4_outputs, num_block=4, kernel_size=3, reduce_dim=64, out_channels=128, is_training=is_training, keep_prob=0.8)

  # # bigblock 6
  # with tf.name_scope("bigblock/6"):
  #   bigblock_6_outputs = bigblock_1d(bigblock_5_outputs, num_block=4, kernel_size=3, reduce_dim=64, out_channels=128, is_training=is_training, keep_prob=0.8)

  # # bigblock 7
  # with tf.name_scope("bigblock/7"):
  #   bigblock_7_outputs = bigblock_1d(bigblock_6_outputs, num_block=4, kernel_size=3, reduce_dim=64, out_channels=128, is_training=is_training, keep_prob=0.8)

  # # bigblock 8
  # with tf.name_scope("bigblock/8"):
  #   bigblock_8_outputs = bigblock_1d(bigblock_7_outputs, num_block=4, kernel_size=3, reduce_dim=64, out_channels=128, is_training=is_training, keep_prob=0.8)

  # FNN
  with tf.name_scope("logit"):
    FNN = tf.layers.conv1d(bigblock_4_outputs, filters=256, kernel_size=1, use_bias=False, kernel_initializer=tf.contrib.layers.xavier_initializer())
